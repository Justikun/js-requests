...
// PROBLEM 2
/*
    Create a function that changes sayHelloButton's background color to black and its 
    text color to white (you can use the .style object or create a CSS class and use classList.add)
    
    Attach a mouseover event to sayHelloButton that calls the function you wrote
*/

// CODE HERE
const buttonChangeToBlack = (button) => {
    button.style.backgroundColor = "black"
    button.style.color = "white"
}

sayHelloButton.addEventListener('mouseenter', () => {
    buttonChangeToBlack(sayHelloButton)
})
...
// CODE HERE
const buttonChangeToWhite = (button) => {
    button.style.backgroundColor = "#EFEFEF"
    button.style.color = "black"
}

sayHelloButton.addEventListener('mouseout', () => {
    buttonChangeToWhite(sayHelloButton)
})
...
const repeatMyParam = () => {
    // YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/Diet_Cola')
    .then(res => {        
...
// CODE HERE
    const myQueryFunc = () => {
        axios.get('http://localhost:3000/query-test/?name1=joe&name2=jess')
        .then(res => {
            console.log(res.data)
        })
    }

    document.querySelector("#query-button").addEventListener('click', myQueryFunc)
...
// PROBLEM 9
/* 
    Back in the ohMy function on Problem 5, replace the console log in the promise's callback with a 
    for loop that loops over res.data. 

    On each iteration of the loop, create a new p element. 
    Set its textContent equal the string at the current index (i) and then append the new p element onto 
    the document's body. 
*/
